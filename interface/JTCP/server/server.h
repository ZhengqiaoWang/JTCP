...
public:
     ...
     * @param cb 回调函数
     */
    void setOnNewClient(OnNewClientCBType cb) noexcept { m_on_new_client_cb = cb; }
...
private:
    ...
    JResultWithErrMsg acceptThreadFunc(const ListenMaxNumType& listen_max_num)
    {
        ...
        m_run_flag = true;
        while (m_run_flag) {
            ready_event_num = epoll_wait(
                m_epollfd, &*event_list.begin(), static_cast<int>(event_list.size()), 1000); // 超时1秒
    ...
    JResultWithErrMsg handleClientMsg(epoll_event& event)
    {
        ...
        if (-1 == ret) {
            ...
            // 从epoll中清除回调
            if (epoll_ctl(
                    m_epollfd, EPOLL_CTL_DEL, peer_client->getFileDescribe()->getFD(), &event) <
                0) {
